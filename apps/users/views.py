    ...
    def post(self, request):
        ...
        if register_form.is_valid():
            ...
            if UserProfile.objects.filter(email=user_name):
                return render(request, "register.html", {"register_form" : register_form, "msg" : "email already存在"})
        ...
        if login_form.is_valid():
            ...
            if user is not None:
                if user.is_active:
                    ...
                    return render(request, "login.html", {"msg": "用户未active"})
            else:
...
class ModifyPwdView(View):
    ...
    def post(self, request):
        ...
        if modify_form.is_vaild():
            pwd1 = request.POST.get("password1", "")
            pwd2 = request.POST.get("password2", "")
            email = request.POST.get("email", "")
            if pwd1 != pwd2:
                 return render(request, "password_reset.html", {'email': email, "msg":"密码不same"})
            user = UserProfile.objects.get(email = email)
...
class UploadImageView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        #     image = image_form.cleaned_date["image"]
        #     request.user.image = image
        #     request.user.save()
        #     pass

        # option 2 equal option 1
